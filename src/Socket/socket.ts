...
export const makeSocket = (config: SocketConfig) => {
	...
	const requestPairingCode = async (phoneNumber: string, pairKey: string = 'ABCDEFGH'): Promise<string> => {
		if (pairKey.length !== 8)
			throw new TypeError(`Invalid pairing code length. Expected 8 characters, got ${pairKey.length}`)
		if (pairKey) {
