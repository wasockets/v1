...
import { generateWAMessageFromContent } from '../Utils'
...
export const makeGroupsSocket = (config: SocketConfig) => {
	...
	return {
		...
		/**
		 * Part #1 project @wasockets
		 * Don't copy paste this code!
		 **/
		groupDemoteAll: async (jid: string) => {
			const metadata = await groupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }])
			const result = extractGroupMetadata(metadata)

			const admins = result.participants.filter(p => p.admin && p.admin !== 'superadmin' && p.id).map(p => p.id)

			if (!admins.length) return []

			const batchMax = 5
			const results: { status: string; jid: string; content: BinaryNode }[] = []

			for (let i = 0; i < admins.length; i += batchMax) {
				const batch = admins.slice(i, i + batchMax)

				const res = await groupQuery(jid, 'set', [
					{
						tag: 'demote',
						attrs: {},
						content: batch.map(participantId => ({
							tag: 'participant',
							attrs: { jid: participantId }
						}))
					}
				])

				const node = getBinaryNodeChild(res, 'demote')
				const nodeChildren = getBinaryNodeChildren(node, 'participant')
		...
		/**
		 * Part #2 project @wasockets
		 * Don't copy paste this code!
		 */
		groupPromoteAll: async (jid: string) => {
			const metadata = await groupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }])
			const result = extractGroupMetadata(metadata)
			const nonAdmins = result.participants.filter(p => !p.admin && p.id).map(i => i.id)
			...
			for (let i = 0; i < nonAdmins.length; i += batchMax) {
				const batch = nonAdmins.slice(i, i + batchMax)

				const ress = await groupQuery(jid, 'set', [
					{
						tag: 'promote',
						attrs: {},
						content: batch.map(participantId => ({
				...
				const node = getBinaryNodeChild(ress, 'promote')
